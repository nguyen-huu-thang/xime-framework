...
| ⭐ **`check_doc_coverage.py`** | **Chiều NGƯỢC LẠI ba script trên: tên công khai nào KHÔNG có một chữ trong `docs/`?** | **146/309 tên** (47%). Lỗ lớn nhất: `core/exception` **18/19**, `core/security` 7/13. ⚠ Hai kết quả **không cân nhau** - *không xuất hiện* là bằng chứng mạnh, *có xuất hiện* là bằng chứng yếu. Ba mã thoát, `2` = **chưa kết luận được** |
| `do_bien_c_python.py` | Vượt biên Python↔C/Rust tốn bao nhiêu? | Lật niềm tin *"code Python ngắn thì bên trong là C nên nhanh"*: Pydantic **793 ns/bản ghi** so với viết tay **230 ns**. Bản đo đầu **sai vì hai vế làm hai lượng việc khác nhau** - nay có `assert` canh |
