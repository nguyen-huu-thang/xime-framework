# Kế hoạch 0.8 bản đầu - Multi-process Runtime + Bus liên Worker

> | | |
> |---|---|
> | **Trạng thái** | ⛔ **BỊ PHỦ ĐỊNH.** Đừng đọc như hiện trạng |
> | **Viết cho** | 0.8, chốt **2026-06-27** |
> | **Bị thay bởi** | Buổi thiết kế **2026-08-16 -> 08-19** |
> | **Giữ lại vì** | nó cho biết **vì sao hồi đó chọn như vậy**, và bốn chỗ buổi 08-16 lật là bốn chỗ đáng học |
>
> ⚠ **0.8 nay ĐÃ CODE XONG** (2026-08-20), theo thiết kế MỚI chứ không theo file này.
> Dòng cũ ghi *"Chưa code. Khi bắt tay code, bổ sung chi tiết vào file này"* đã hết đúng
> theo hai chiều: bản này không bao giờ được code, còn bản thay nó thì đã xong.
>
> **Đọc thay:**
>
> | | |
> |---|---|
> | [`../thiet-ke/10-da-tien-trinh.md`](../thiet-ke/10-da-tien-trinh.md) | Mô hình chạy, `main.py`, cấu hình, adapter |
> | [`../thiet-ke/11-bus-lien-tien-trinh.md`](../thiet-ke/11-bus-lien-tien-trinh.md) | **Bus - thay hẳn phần Bus của file này** |
> | [`../thiet-ke/12-kho-refdata.md`](../thiet-ke/12-kho-refdata.md) | Kho nhóm 1 - `RefData` |
> | [`../thiet-ke/13-kho-store-lmdb.md`](../thiet-ke/13-kho-store-lmdb.md) | Kho nhóm 2 - `Store` trên LMDB |
> | [`../thiet-ke/09-kho-lien-tien-trinh-boi-canh.md`](../thiet-ke/09-kho-lien-tien-trinh-boi-canh.md) | Bối cảnh hai kho, và **mục 2.7** phạm vi một máy |
>
> **Ba thứ trong file này KHÔNG còn dùng:**
>
> | Bản 2026-06-27 | Thay bằng |
> |---|---|
> | Bus Manager + **một queue chung** + mutex + transport abstraction + API broadcast | **Bộ nhớ chung, mỗi tiến trình một vùng ghi riêng, cha KHÔNG nằm trên đường đi** |
> | DI scope `global` / `worker`; Worker 0 giữ global singleton; Worker 0 chết thì **crash toàn cụm** | **Bốn tầng**, và primary **thăng cấp được** khi con thứ nhất chết |
> | HTTP routing tới worker | Không cần - **mỗi tiến trình một cổng**, hoặc cha giữ socket, hoặc `SO_REUSEPORT` |
>
> **Còn dùng được:** `BusMessage`, phần hàng đợi, và mục *"Mảng 2 - Config"*.
