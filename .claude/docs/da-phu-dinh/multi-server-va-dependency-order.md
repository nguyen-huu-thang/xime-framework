# Multi-server và `dependency.order()` - kế hoạch hai tính năng

> | | |
> |---|---|
> | **Trạng thái** | ⚠ **NỬA BỊ PHỦ ĐỊNH** - phần 1 hết đúng, phần 2 vẫn đúng |
> | **Viết cho** | thời 0.6-0.7; cả hai phần đều đã code xong khi đó |
> | **Bị thay bởi** | 0.8 (chốt 2026-08-19) - **chỉ phần 1** |
>
> ⛔ **Phần 1 - Multi-server: ĐỪNG LÀM THEO.** Nó dạy
> `app.use(WebAdapter("admin", "0.0.0.0", 8081))`, mà **0.8 cấm hẳn đối số địa chỉ trong
> code**: `WebAdapter.__init__` nay chỉ còn `server_id`. Địa chỉ đến từ cấu hình -
> `process.web.<id>` cho một tiến trình, `processes.<p>.web.<id>` cho nhiều.
> Hiện trạng: [`../thiet-ke/14-api-adapter.md`](../thiet-ke/14-api-adapter.md) mục 2, và
> [`../thiet-ke/10-da-tien-trinh.md`](../thiet-ke/10-da-tien-trinh.md).
>
> ✅ **Phần 2 - `dependency.order()`: VẪN ĐÚNG và đang chạy.** `XimeContainer.order()` có
> thật, hành vi đúng như mô tả dưới đây. Nó nằm trong thư mục này chỉ vì chung file với
> phần 1 - **đừng đọc cả file thành đồ bỏ**.
>
> 📌 Phần 1 là ca thật của *một quyết định đúng ở bản này thành sai ở bản sau*: ép khai địa
> chỉ trong code từng là cách chặn server phụ chạy nhầm cổng, rồi cha-giữ-socket ra đời và
> biến chính dòng code đó thành nguồn nói dối.

> Ký hiệu trong file: `[ ]` chưa làm · `[x]` đã xong · `[~]` đang làm
