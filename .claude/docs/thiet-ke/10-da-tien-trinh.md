...
>
> Cặp với [`09-kho-lien-tien-trinh-boi-canh.md`](09-kho-lien-tien-trinh-boi-canh.md)
> (cùng buổi, nửa đầu: cache và LMDB). File này là nửa sau: **mô hình chạy**.
>
> ⚠ Cả hai file cộng lại làm **phần lớn [`../da-phu-dinh/ke-hoach-0.8-ban-dau.md`](../da-phu-dinh/ke-hoach-0.8-ban-dau.md) không
...
> ✅ **ĐÃ THI CÔNG 2026-08-20** (`xime/core/bootstrap/_watchdog.py`).
>
> ⚠ **Một chỗ mục này nói sai, sửa lúc thi công: nhịp vỗ KHÔNG đi bằng `ProcessLink`.**
> Câu *"watchdog đi chung chuyến đó"* đúng ở tầng khái niệm nhưng số học thì không:
> nhịp 1 giây × 4 tiến trình đổ vào một vòng 256 dòng thì nó **vòng lại sau một
> phút**, và vì không ai đọc nhịp của người khác nên mỗi lần vòng lại **cộng vào
> `missed`** - chỉ số chẩn đoán chính của bus. Nhịp vỗ là một **đại lượng bị ghi
> đè**, không phải một **sự kiện**; nó ở một vùng nhớ riêng, `16 + 8×N` byte.
>
> ⚠ **Và mục này thiếu một ngưỡng**: nó nói `NEVER` nghĩa là *đang khởi động*
> nhưng không nói khi nào thì đang-khởi-động thôi là một lời bào chữa. Thi công
> thêm `STARTUP_GRACE_SECONDS = 60`.
...
> ```
>
> Đây **chính là pha** mà [tài liệu snapshot](12-kho-refdata.md) mục 6
...
> ⭐ **Mảng này nay có tài liệu riêng:** [`14-api-adapter.md`](14-api-adapter.md).
...
> ### ⚠⚠ BỔ SUNG 2026-08-20: mục này ĐÚNG nhưng KHÔNG ĐỦ
>
> Nó phủ app *một tiến trình, một cổng mỗi loại*. Không phủ app **một tiến trình,
> HAI server web** - thứ vẫn hợp lệ vì `server_id` là điểm phục vụ **bên trong**
> một tiến trình (nguyên lý 3), nên nó độc lập với `share_load()`.
>
> Ca đó trước 0.8 khai địa chỉ bằng **đối số constructor**, mà phần 2 của mảng
> adapter lại bỏ hẳn đối số. Chủ dự án chốt 2026-08-20: thêm khoá **`process:`** -
> một khối, nội dung **giống hệt** một khối của `processes:`, trừ ba khoá chỉ có
> nghĩa khi có nhiều tiến trình (`primary`, `count`, `shared`).
>
> ```yaml
> process:
>   web:
...
> ```
>
> Khoá phẳng `server:` / `grpc.port` **giữ nguyên hiệu lực**, được dịch thành
> `process.web.default`. Chi tiết ở mục **2.5b** của
> [`14-api-adapter.md`](14-api-adapter.md).
...
Nhưng [phần 5 của mảng đổi API adapter](14-api-adapter.md) chuyển
...
|---|---|---|---|
| **web** (uvicorn) | **cha giữ socket, truyền fd cho con** | ✅ | ✅ nhưng **KHÔNG phải nhờ `WSADuplicateSocket`** - xem 5.7.1b |
...
#### 5.7.1b. ⚠⚠ Ô "Windows ✅" của web ĐÚNG KẾT QUẢ, SAI CƠ CHẾ (đo 2026-08-20)

> Phát hiện lúc thi công giai đoạn 3. Bảng trên **vẫn đúng ở kết luận** (web dùng
> chung cổng được trên Windows), nhưng lý do nó khai ra thì không đủ, và thiếu
> đúng mảnh khiến nó không chạy.

`WSADuplicateSocket` chuyển handle qua được thật - `multiprocessing` làm việc đó
đàng hoàng. Chỗ vỡ nằm ở tầng khác: `asyncio` mặc định trên Windows là
**proactor**, và ở đó lần `accept()` đầu tiên gọi `CreateIoCompletionPort`:

```text
OSError: [WinError 87] The parameter is incorrect
  File "asyncio\windows_events.py", line 713, in _register_with_iocp
```
...
> vào IOCP của mình nữa**, dù nó cầm một handle hoàn toàn hợp lệ.

⭐ **Cách hỏng của nó là kiểu tệ nhất trong ba kiểu**: con thứ hai khởi động
**thành công**, log *"serving"*, rồi **không nhận nổi một kết nối nào**. Cụm mất
một nửa năng lực trong khi mọi request đều 200, không log lỗi nghiệp vụ nào, và
không có gì đỏ. Đo thật lần đầu: hai con, một con trả lời 40/40 request.

⛔ **`sock.share(os.getpid())` + `fromshare()` KHÔNG cứu được** - và điều này đáng
ghi vì đó **đúng cách uvicorn làm** ở chế độ nhiều worker (`_share_socket`), tức
một lời giải trông có thẩm quyền. Đã đo cả hai đường (`plain` và `reshare`), cùng
một lỗi ở cùng một dòng.

✅ **Thứ cứu được: selector event loop.** Nó gọi `accept()` thẳng, không đụng IOCP.
Đo lại: hai tiến trình cùng trả lời trên một cổng, pid khác nhau.

| Đo (Windows, Python 3.14) | Kết quả |
|---|---|
| proactor, socket kế thừa nguyên | 1/2 tiến trình phục vụ, con kia `WinError 87` |
| proactor, `share()` + `fromshare()` | **y hệt** - vẫn 1/2 |
| **selector** | **2/2 tiến trình phục vụ**, kernel chia request |

**Bản vá đã thi công:** tiến trình con nào **kế thừa socket dùng chung** trên
Windows thì chạy trên selector loop, kèm một dòng `WARNING` nói rõ vì sao
(`worker_loop_factory`). Không kế thừa socket thì giữ nguyên proactor.

⚠ Cái giá: `select()` trên Windows giới hạn **512 socket**, và loop đó không chạy
được subprocess. Chấp nhận vì **Windows là máy dev**, prod là Linux (ở đó proactor
không tồn tại và `epoll` không có giới hạn này). Đổi lại giữ được thứ đắt hơn
nhiều, đúng thứ mục 5.6 đã trả giá để có: **dev chạy giống prod**.

📌 Bài học chung hơn con số: ô ✅ đó dựa trên *"handle chuyển qua được"*, và câu đó
**đúng**. Nhưng chuyển được handle và **dùng được** handle là hai chuyện, và bảng
...
>
> ✅ **Phần khai chữ ký ĐÃ LÀM 2026-08-20**: bỏ `device=` khỏi `@poll`/`@on_change` và
> `server=` khỏi `@on_node_change` · tham số handler `device`/`server` khớp theo TÊN ·
> `devices_of()` / `servers_of()`. Chi tiết + chỗ OPC UA dùng chữ khác:
> [`14-api-adapter.md`](14-api-adapter.md) mục 4b.7.
...
✅ **Chủ dự án chốt: dùng BUS liên tiến trình** (thứ `../da-phu-dinh/ke-hoach-0.8-ban-dau.md` thiết kế sẵn mà
...
>
> **[`11-bus-lien-tien-trinh.md`](11-bus-lien-tien-trinh.md)** - thiết
...
[file cache](09-kho-lien-tien-trinh-boi-canh.md). Ranh giới tự nó rõ.
...
⚠ Phần lớn `../da-phu-dinh/ke-hoach-0.8-ban-dau.md` đã bị lật (mục 7), nên cái cần là **viết lại bus với vai
...
## 7. Ảnh hưởng tới `../da-phu-dinh/ke-hoach-0.8-ban-dau.md`

Cộng với file cache cùng buổi, **phần lớn bản kế hoạch đó không còn cần**:

| Thành phần trong 0.8 | Còn cần |
|---|---|
| Bus Manager · shared queue + mutex · `BusMessage` · transport abstraction · API `broadcast` | **Không.** Bus **giữ lại nhưng đổi hẳn cơ chế**: bộ nhớ chung, mỗi tiến trình một vùng ghi riêng, **cha không nằm trên đường đi**. Thiết kế đầy đủ chốt 2026-08-18: [`11-bus-lien-tien-trinh.md`](11-bus-lien-tien-trinh.md) |
...
> **Đề nghị: `../da-phu-dinh/ke-hoach-0.8-ban-dau.md` nên được VIẾT LẠI chứ không bổ sung.**
...
| ~~C~~ | ✅ **VIẾT XONG 2026-08-19**: [`rules/module-level-code.md`](../../rules/module-level-code.md). ⭐ Phần đáng nhớ nhất **không phải kết nối thừa** mà là **giá trị không tất định**: `uuid4()` ở mức module cho mỗi tiến trình một giá trị **khác nhau** trong khi code đọc nó tin là dùng chung - đó không phải lãng phí, đó là **sai**. Cùng khuôn `hash()` ở kho nhóm 2. ✅ **Hai phép dò HIỆN THỰC XONG 2026-08-20** (giai đoạn 7): `share_load()` đo thời gian + `xime check module-level`. ⚠ **Ngưỡng 1 giây của kế hoạch ĐO RA LÀ SAI** - hai app thật và lành mạnh tiêu 0,99s và 1,05s, nên nâng lên **3 giây**; xem `rules/module-level-code.md` mục 4.1b | 📌 Bus cho một ví dụ: mở `ProcessLink` ở mức module là **N+1 lần attach vùng nhớ chung**, và không gì báo |
...
| ~~D~~ | ~~**Bus liên tiến trình: thiết kế lại theo vai trò mới**~~ | ✅ **XONG 2026-08-18** - [`11-bus-lien-tien-trinh.md`](11-bus-lien-tien-trinh.md). Tên chốt **`ProcessLink`**. ⚠ Hai kết luận của 5.7.4b đã bị lật: **bộ nhớ chung chứ không phải kênh cha-con**, và **cha KHÔNG nằm trên đường đi** |
...
| ~~9~~ | ~~Tên thống nhất cho đối số định danh của cả sáu adapter~~ | ✅ **ĐÓNG 2026-08-19: KHÔNG ép cùng một tên**, đúng như ghi chú này ngờ. Hai tên ở bề mặt (`server_id` · `target_id`) cộng **một tên chung ở tầng framework** (`adapter_id`). Chi tiết: [`14-api-adapter.md`](14-api-adapter.md) mục 1 |
...
- [`09-kho-lien-tien-trinh-boi-canh.md`](09-kho-lien-tien-trinh-boi-canh.md) - nửa
  đầu cùng buổi: nhóm 1 (shared memory), nhóm 2 (LMDB), lý do hoãn đa luồng.
- [`../da-phu-dinh/ke-hoach-0.8-ban-dau.md`](../da-phu-dinh/ke-hoach-0.8-ban-dau.md) - **nên viết lại**, xem mục 7.
- [`03-diem-khoi-dong.md`](03-diem-khoi-dong.md) - khuôn `main.py` hiện hành. Mục 5 ở
