...
>
> Khác với ba đợt kiểm toán trước (`0.5.md`, `0.6.md`, `0.7.md`) vốn hỏi
...
> [`0.7-bao-mat-ke-hoach-va.md`](0.7-bao-mat-ke-hoach-va.md) - 5 đợt, code cụ thể
...
> framework còn lại **chưa vá**. Chi tiết: [`../phien-ban/0.7.1-ket-qua.md`](../phien-ban/0.7.1-ket-qua.md).

> ⭐ **Đo lại 2026-08-27 bởi phiên `xime framework`, và cột trạng thái SAI ở BA
> dòng** - cả ba đều sai theo chiều *bi quan*, tức việc đã làm mà sổ vẫn ghi là
> nợ. Không dòng nào ghi "đã vá" mà thực ra chưa.
>
> | Mã | Sổ ghi | Đo được |
> |---|---|---|
> | **F10** | ➡ dời sang 0.8 | ✅ **đã vá ở `0.8.0`** - `_run_adapters` khai thẳng *"Không dùng `asyncio.TaskGroup`"*, adapter lỗi bị cô lập + log `CRITICAL` + trạng thái `ISOLATED` trong `health()`. Đúng "hướng cân bằng" mục F10 đề xuất |
> | **A6** | ⬜ chưa vá | ✅ **đã vá** - `Monolithic/shop/backend/resources/application*.yml` nay nói thẳng file đó không bao giờ được nạp và **dẫn đích danh A6**. 5 app Monolithic kia chưa từng nhắc tới nó, workspace Xime cũng không |
> | **A7** | ⬜ chưa vá | 🟡 **vá một phần: 3/12** giá trị đã xoay sang chuỗi ngẫu nhiên thật (`cho-thue-thiet-bi`, `shop-hoa-qua-tang`, + 1). **9 còn theo khuôn** `s3cr3t-callback-<tên app>` |
>
> ⭐⭐ **Hệ quả đáng nhớ hơn ba dòng đó: mọi mục FRAMEWORK nay đã đóng (17/17).**
> F10 là cái cuối cùng, và nó đóng từ `0.8.0` - tức suốt hơn một tuần sổ này ghi
> framework còn nợ trong khi thực tế không còn. Phần chưa làm của kiểm toán
> **nằm trọn ở tầng ứng dụng**.
>
> ⚠ Ba đánh giá trên do **người đứng ngoài** đo. Theo khuôn 18 của sổ điều phối,
> phiên đang giữ repo đo lại mới là kết luận cuối - nhất là A7, nơi tôi chỉ đọc
> giá trị trong `application.yml` chứ không kiểm bên payment có khớp không.
...
| **A3** | 🟠 CAO | Sáu app Monolithic ký JWT HS256 bằng **cùng một secret literal**; ở `shop` nó nằm trong git và app đã deploy | 6 app | ⬜ **chưa vá, xác nhận lại 2026-08-27** - 6/6 app vẫn dùng chung `dev-secret-CHANGE-IN-PRODUCTION-use-32chars-minimum` |
...
| **F10** | 🟡 TRUNG | Một adapter chết kéo sập cả tiến trình (`TaskGroup`) | framework | ✅ **vá ở `0.8.0`** (đo lại 2026-08-27) - trước đó ghi ➡ **DỜI SANG 0.8** - nó mở rộng `Adapter` protocol (đổi API cho cả adapter người dùng tự viết), và supervisor của 0.8 cần đúng tín hiệu "ready" đó |
| **F11** | 🟡 TRUNG | `audience` mặc định KHÔNG ép | framework | ✅ vá 0.7.1 |
| **A4** | 🟡 TRUNG | `/docs`, `/redoc`, `/openapi.json` nằm trong `public_paths` | toàn bộ app **+ 4 service Java** | 🟡 **framework vá 2026-08-27** (`xime.dev`, mặc định TẮT) · ⬜ **24 file `application.yml` còn khai** · ⬜ **4 service Java `agent` `identity` `payment` `user` phơi springdoc** - phần này A4 **chưa từng đếm**, xem mục A4 |
| **A5** | 🟡 TRUNG | 6 app Monolithic: thiếu header Authorization -> đi tiếp **ẩn danh** | 6 app | ⬜ **chưa vá, xác nhận lại 2026-08-27** - 6/6 còn nguyên dòng `jwt_middleware.py:57` |
| **A6** | 🟡 TRUNG | Tài liệu bảo để secret vào `application-secret.yml` - framework **không nạp file đó** | shop + khuôn mẫu | ✅ **đã vá** (đo lại 2026-08-27) - chú thích ở `shop` đã đính chính và dẫn đích danh A6 |
...
| **A7** | ⚪ THẤP | `callback_secret` là chuỗi literal yếu, giống nhau theo khuôn mẫu | nhiều app | 🟡 **vá một phần 3/12** (đo lại 2026-08-27) - 9 giá trị còn theo khuôn `s3cr3t-callback-<tên app>` |
...
### ✅ Phía framework: vá 2026-08-27 - một công tắc `xime.dev`, mặc định TẮT

Ba đường đó nay **không tồn tại** trừ khi ứng dụng khai `xime.dev: true`. Đề xuất gốc ở trên
(*"tắt hẳn bằng `configure_openapi(docs_url=None, ...)`"*) **đã bị thay bằng hướng khác**, vì
nó có hai chỗ hỏng: nó bắt mỗi app tự nhớ tắt (đúng cách A1 sống sót), và nó tách một quyết
định **theo môi trường** thành ba khoá **theo tính năng**, mà đặt `docs_url` mà quên
`openapi_url` thì được một trang hỏng chứ không phải một trang tắt.

⭐ Chỗ đáng ghi hơn cả bản vá: **giấu `/docs` sau xác thực không phải đường thay thế.** Swagger
UI là trang mở bằng trình duyệt, mà trình duyệt không gắn header `Authorization` khi gõ URL -
nên `/docs` nằm ngoài `public_paths` trả 401 cho đúng người muốn đọc nó. Đó là lý do 24 repo
đều khai nó công khai: **không ai làm sai cả, cách duy nhất để dùng được nó là mở nó ra.** Một
lỗ hổng mà mọi người đều rơi vào thì nguyên nhân nằm ở thứ họ được cho, không ở kỷ luật của họ.

### ⬜ Phía app Python: 24 file còn khai (đo 2026-08-27)

Phân tích YAML thật, **0 file không đọc được**: **24** file `application*.yml` (24 repo, mỗi
repo một file) còn khai ba đường trong `auth.jwt.public_paths`. Nay **vô hại về hành vi** -
route không tồn tại thì `public_paths` không bao giờ được tra tới - nhưng là **lời khai sai**:
người mở cấu hình ra đọc sẽ tin ba đường đó đang mở.

Và **0/55** file có khoá `xime.dev`, nên hôm nay không máy dev nào còn `/docs`. Repo nào cần thì
thêm một dòng vào yml dev của mình.

⭐ Sửa `saas-foundation/template` **trước**: nó nằm trong 24 file đó, tức vẫn đang phát tán khuôn
cũ cho mọi app clone về sau. Cùng thứ tự ưu tiên đã dùng cho A1 - *nợ đứng yên để sau, nợ đang
sinh thêm thì chặn ngay*.

📌 `shop-hoa-qua-tang` là repo **duy nhất** từng tự xử lý A4 (cờ `auth.jwt.expose_docs` trong
`config/jwt.py`). Cơ chế đó nay thừa nhưng vô hại - đừng vội gỡ, nó là lớp thứ hai.

### ⬜⭐ Phía Java: bốn service Base, và A4 CHƯA TỪNG ĐẾM chúng (phát hiện 2026-08-27)

| Service | `springdoc` trong `pom.xml` | cấu hình tắt | đường swagger |
|---|---|---|---|
| `agent` · `identity` · `payment` · `user` | **có** | **0 dòng** ở cả bốn repo | `permitAll()` tường minh trong `SecurityConfiguration.java` |

Bốn service này phục vụ `/swagger-ui.html` và `/v3/api-docs/**` **không cần xác thực** - đúng
phơi bày mà A4 mô tả, ở bốn service **hạ tầng** thay vì ở app nghiệp vụ. Bản vá framework không
với tới: khác ngôn ngữ, khác cơ chế. Tắt thì rẻ:
...
⭐⭐ **Vì sao chúng lọt lưới suốt từ 2026-08-01, và đây mới là phần đáng học:** A4 mô tả vấn đề
**bằng một khối YAML `public_paths`** - hình dạng của Python. Một phép dò dựng theo hình dạng đó
**không có ô nào** cho Spring Security, nên bốn repo này không nằm ở cột "đã vá" cũng không nằm
ở cột "chưa vá" - chúng không nằm ở cột nào cả. Cùng khuôn với ca `admin` tự dựng gRPC server
bằng tay mà bảng khảo sát đa tiến trình không có chỗ xếp.

Và cột phạm vi của A4 ghi *"toàn bộ app"* - câu đó **đọc như đã bao trùm**, nên không ai đi tìm
thêm. Một nhãn phạm vi rộng hơn phép đo sinh ra nó là thứ mua sự yên tâm cho một vùng chưa ai
nhìn, y hệt nhãn *"đã quét toàn workspace"* của đợt quét secret 2026-08-03.
...
> [`../ghi-chep/go-phu-thuoc-khai-niem.md`](../ghi-chep/go-phu-thuoc-khai-niem.md)). Thay bằng
