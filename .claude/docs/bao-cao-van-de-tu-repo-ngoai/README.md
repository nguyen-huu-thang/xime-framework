...
| [Hai hàm đọc danh tính peer dễ chọn nhầm nhau](data-hai-ham-doc-danh-tinh-peer-de-chon-nham-2026-08-26.md) | 2026-08-26 | ✅ **ĐÃ ĐỌC · ĐO LẠI XÁC NHẬN ĐÚNG · ĐÃ VÁ** 2026-08-26. Không phải lỗi code - đã đọc cả phần trích xuất `_read_peer_cn`/`_read_peer_sans`, cả hai fail-soft đúng chiều **an toàn** (không có danh tính thì authorizer từ chối). ⭐ Đo lại thì phạm vi rộng hơn, và lần này nó **nặng hơn một bậc chứ không chỉ rộng hơn**: báo cáo nói *tài liệu không nói phải CHỌN*, nhưng `docs/{en,vn}/core-concepts.md` **khẳng định thẳng lựa chọn sai** - *"CN ... may be a service id **or an application identity**"*, ngược hẳn docstring của chính interceptor (`_context.py:113`: *"PEER_CN identifies the calling **process**"*). `git log -S` ra câu đó có từ **`c90586d v0.4.0`**, sống qua **ba** lần đổi API (`current_app_id()` thêm ở `v0.6.3`, gỡ ở `v0.7.1`, `current_peer_sans()` vào thay chỗ) mà không ai mở file tài liệu. Thêm hai thứ báo cáo không thấy: `current_peer_sans()` có **0 lần** xuất hiện trong `docs/` dù là **bản thay thế** cho một API đã gỡ, và `check_doc_coverage.py` **đã kêu từ trước** (`core.security ... thieu: current_peer_sans, PEER_CN, PEER_SANS`). ⇒ Nhận bản vá họ đề nghị y nguyên thì vá **nửa nhẹ**, để nguyên nửa nặng. Đã sửa cả ba: câu sai ở EN+VN · đoạn chéo hai chiều trong hai docstring · mục đầy đủ cho `current_peer_sans()` + hai key `PEER_CN`/`PEER_SANS`. Đo: `core.security` từ **7 thiếu → 4** (EN) và **→ 5** (VN); 2625/24/0 = **2649**, không hồi quy. Đề nghị của họ **trung lập, không kéo lại** phụ thuộc khái niệm đã gỡ ở `0.7.1` - *"CN là một peer, danh tính chung bền hơn nằm ở SAN"* là sự thật PKI phổ quát, SPIFFE/SPIRE làm đúng vậy `current_caller()` (CN = một peer cụ thể) và `current_peer_sans()` (SAN = danh tính logic bền hơn) trả lời hai câu khác nhau, nhưng docstring của cả hai đều quy về *"ai gọi"*, nên chốt phân quyền cầm nhầm cái ngắn hơn. Ca thật ở `data`: chốt *"chỉ app admin"* so theo CN tiến trình, sống 2 tháng, hỏng **hai chiều** (chặn oan tiến trình thứ hai · im lặng đi theo mọi thay đổi ở tầng cấp cert). Người báo **không quét repo khác** và nói rõ điều đó |
...
Đây là quan sát đáng giá nhất của cả thư mục, và tới 2026-08-26 nó đúng **8/8
...
| docstring **không nói** phải chọn giữa `current_caller()` và `current_peer_sans()` (thiếu một đoạn) | tài liệu người dùng **khẳng định thẳng lựa chọn sai**, ngược với docstring của chính framework. Có từ `v0.4.0`, sống qua **ba** lần đổi API. Người báo chỉ đọc docstring, chưa mở `docs/` - nên bản vá họ đề nghị vá đúng **nửa nhẹ**. ⭐ Lần đầu phạm vi rộng ra theo chiều **mức độ**, không phải theo chiều **số lượng** |
