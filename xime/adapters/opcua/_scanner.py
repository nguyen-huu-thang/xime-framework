"""Controller discovery for the OPC UA adapter.

No separate routing package here: unlike Modbus there is nothing to group (OPC
UA pushes per node, so there are no intervals to reconcile), and a builder that
only makes a list does not earn its own module.
Không tách package routing vì OPC UA đẩy theo từng node - không có nhịp nào để
gom, nên builder chỉ là một danh sách.
"""
...
from ._decorators import OPCUA_ATTR


class OpcuaControllerScanner:
    """Finds classes with at least one method carrying OPCUA_ATTR."""
    ...
    def _scan_package(self, package_name: str) -> list[type]:
        try:
            ...
            raise ImportError(
                f"Cannot scan OPC UA controller package '{package_name}': {exc}"
            ) from exc
