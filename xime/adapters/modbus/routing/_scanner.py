...
from .._decorators import MODBUS_ATTR


class ModbusControllerScanner:
    """Walks packages and finds Modbus controller classes.
    ...
    def _scan_package(self, package_name: str) -> list[type]:
        try:
            ...
            raise ImportError(
                f"Cannot scan Modbus controller package '{package_name}': {exc}"
            ) from exc
